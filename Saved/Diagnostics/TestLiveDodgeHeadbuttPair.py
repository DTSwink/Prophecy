...
folder=pathlib.Path(unreal.Paths.project_saved_dir())/'Diagnostics/LiveDodgeHeadbuttPair'
...
def tick(dt):
    try:
        ...
        if s['phase']=='start':
            ...
            d.hide_sword();assert not d.get_held_sword()
        ...
        if s['phase']=='settle' and t-s['time']>.3:
            a,d=s['attacker'],s['defender']
            assert a.trigger_nn_attack('headbutt',point(d,'spine_03'),False),'Attack failed'
