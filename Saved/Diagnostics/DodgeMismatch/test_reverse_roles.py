...
def finish(error=''):
 ...
 p=pathlib.Path(unreal.Paths.project_saved_dir())/'Diagnostics/CombatReverseRoles/PIE.json';p.parent.mkdir(parents=True,exist_ok=True);p.write_text(json.dumps({'error':error,'rows':s['rows']},separators=(',',':')))
...
def tick(dt):
 try:
  ...
  if 'p' not in s:
   ...
   s.update(p=p,o=o)
  p,o=s['p'],s['o'];reverse=t>=3.1;dodge=t>=6.1
