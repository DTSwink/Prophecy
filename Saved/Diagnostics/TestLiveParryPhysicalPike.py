...
folder=pathlib.Path(unreal.Paths.project_saved_dir()) /'Diagnostics/LiveParryPhysicalPike'
...
def finish(result,error=''):
    ...
    report={k:v for k,v in s.items() if k in ('rows','agents','initial_distance_cm','max_hand_motion_cm','max_attacker_hand_motion_cm','final_steps')}
    report.update(result=result,error=error,scope='Two actual testNN PIE actors, live attack and parry NN, Jolt physical following, Pike sword thrust; no prerecorded poses.')
    (folder/'PIE.json').write_text(json.dumps(report,indent=2)+'\n')
    print('LIVE_PARRY_PAIR',result,error)
def tick(dt):
    try:
        ...
        if s['phase']=='settle' and t-s['time']>.2:
            ...
            assert api.start_nn_parry(d,a,unreal.ProphecyParryBlocker.BLADE,2.0) is not None,'Parry start failed'
