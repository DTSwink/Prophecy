...
static TAutoConsoleVariable<int32> CVarHandCOMTest(TEXT("Prophecy.Jolt.Diagnostic.COMTarget"), 0, TEXT("Temporary COM target conversion trial."));
    ...
    FVelocityServo::FSample& Sample, FVelocityRewrite& Rewrite)
{
    ...
    // Retain the target/h/strength controller, body-origin error and COM velocity semantics.
    // The resulting velocities use Jolt's native limits and activation policy.
    if (Target.LinearStrength > 0.0f)
    {
        FVector Error = TargetPositionCm - Sample.PositionCm;
        if (CVarHandCOMTest.GetValueOnAnyThread() == 1)
        {
            const FVector Offset = FromJoltLinearVelocity(Body.GetShape()->GetCenterOfMass());
            Error += TargetRotation.RotateVector(Offset) - Sample.Rotation.RotateVector(Offset);
        }
        const FVector Desired = Error / DenominatorSeconds;
    ...
    if (CVarHandCOMTest.GetValueOnAnyThread() == 2 && Target.LinearStrength > 0.0f)
    {
        const FVector Offset = Sample.Rotation.RotateVector(FromJoltLinearVelocity(Body.GetShape()->GetCenterOfMass()));
        Linear += Angular.Cross(Offset) * Target.LinearStrength;
    }
