...
s={'rows':[],'start':time.monotonic(),'paused':False,'passed_pause':True};builtins._sword_jump_observation=s
...
def finish(err=''):
 ...
 pathlib.Path(unreal.Paths.project_saved_dir(),'Diagnostics/SwordJump_slop01_observed.json').write_text(json.dumps(dict(rows=s['rows'],error=err)))
...
def tick(dt):
 try:
  ...
  s['rows'].append(r)
  if r['tick']>=60 and not s.get('slop_applied'):
   p=a.get_physical_body_state('hand_r')[0].translation
   unreal.SystemLibrary.execute_console_command(w,'Prophecy.Jolt.ContactExperiment slop 0.1 at %.9f %.9f %.9f'%(p.x,p.y,p.z))
   s['slop_applied']=True
   print('SWORD_SLOP_CHANGED',r['tick'])
