...
class KneeFootAlignmentCapture:
    def __init__(self):
        ...
        self.out=pathlib.Path(unreal.Paths.project_saved_dir())/'Diagnostics'/('KneeFootAlignment-'+time.strftime('%Y%m%d-%H%M%S'))
        ...
        try:
            ...
            for a in self.actors:
                ...
                for side in ('l','r'):
                    ...
                    row={'frame':self.n,'time':unreal.GameplayStatics.get_time_seconds(w),'agent':name,'side':side,'attack':attack,'mode':str(a.get_simulation_mode()),'alpha':alpha,'metrics':metrics}
                    ...
                    if self.n>=60:
                        ...
                        if side=='l' and 'physical' in metrics:
            ...
            if self.n>=1500 or time.monotonic()-self.first>=60:self.finish()
