// Experimental geometry contracts: include after ProphecyLowerTempering.inl in its namespace.
...
struct FScopedCandidate
{
    ...
    FScopedCandidate() { if (Variable) Variable->Set(1,ECVF_SetByCode); }
    ~FScopedCandidate() { if (Variable) Variable->Set(Previous,ECVF_SetByCode); }
#endif
};
