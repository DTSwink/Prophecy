...
def state(a):
 ...
 return {'name':a.get_name(),'attack':[str(x[0])]+list(x[1:]) if x else None,'activity':str(unreal.ProphecyNNDefenseLibrary.get_agent_state(a)),'defense':dict(active=d.active,steps=d.completed_steps,frame=d.attacker_frame,contact=str(d.contact_collider)) if d else None}
def finish(error=''):
 unreal.unregister_slate_post_tick_callback(s['cb']);level.editor_request_end_play()
 p=pathlib.Path(unreal.Paths.project_saved_dir())/'Diagnostics/RemoveParryBlocker/PIE.json';p.parent.mkdir(parents=True,exist_ok=True);p.write_text(json.dumps({'error':error,'rows':s['rows']},separators=(',',':')))
...
def tick(dt):
 try:
  ...
  if 'p' not in s:
   ...
   s.update(p=p,o=o)
  p,o=s['p'],s['o'];reverse=False;dodge=t>=2.5
  p.set_editor_property('CombatDemoReverseRoles',reverse);p.set_editor_property('CombatDemoUseDodge',dodge)
  s['rows'].append(dict(t=t,reverse=reverse,dodge=dodge,player=state(p),opponent=state(o)))
  if t>=4.8:finish()
