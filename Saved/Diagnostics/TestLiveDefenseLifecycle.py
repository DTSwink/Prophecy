...
folder=pathlib.Path(unreal.Paths.project_saved_dir())/'Diagnostics/LiveDefenseLifecycle';folder.mkdir(parents=True,exist_ok=True)
...
def note(label):
    ...
    s['rows'].append(dict(phase=label,steps=[v.completed_steps for v in values],active=[v.active for v in values],attacker_frames=[v.attacker_frame for v in values],checkpoint_weights=[list(a.get_locomotion_checkpoint_weights()) for a in s['defenders']]))
def finish(result,error=''):
    ...
    print('LIVE_DEFENSE_LIFECYCLE',result,error)
def tick(dt):
    try:
        ...
        if s['phase']=='start':
            ...
            d0.set_locomotion_policy_blend_times(0.,0.);d1.set_locomotion_policy_blend_times(0.,0.)
            d0.set_locomotion_input(unreal.Vector(),False,unreal.Vector(),1.,1.)
            d1.set_locomotion_input(unreal.Vector(),True,unreal.Vector(),1.,1.)
            d0.set_locomotion_walk_checkpoint_speed_threshold(-1.)
            d1.set_locomotion_walk_checkpoint_speed_threshold(-1.)
            s.update(attacker=a,defenders=[d0,d1],phase='settle',time=t)
        if s['phase']=='settle' and t-s['time']>.2:
            a=s['attacker'];d0,d1=s['defenders']
            target=d0.get_authored_body_world_target('spine_03')[1].translation
            assert a.trigger_nn_attack('slashRU',target,False)
            assert d0.get_locomotion_checkpoint_weights()[0]>.999
            assert d1.get_locomotion_checkpoint_weights()[1]>.999
            assert api.start_nn_dodge(d0,a,3) is not None
            assert api.start_nn_dodge(d1,a,3) is not None
            s.update(attacker=a,defenders=[d0,d1],phase='both',time=t);note('two dodges started')
        ...
        if s['phase']=='both' and min(v0.completed_steps,v1.completed_steps)>=3:
            ...
            assert status(d0).completed_steps==0;s.update(phase='restart');note('first replaced with parry')
            ...
            note('mixed dodge and parry')
            assert api.start_nn_dodge(d0,a,3) is not None
            assert status(d0).completed_steps==0;s.update(phase='dodge_again');note('parry replaced with fresh dodge')
        elif s['phase']=='dodge_again' and v0.completed_steps>=3:
            assert v0.active and v1.active and v0.attacker_frame==v1.attacker_frame==a.get_nn_attack_state()[4]
            note('two dodges after replacement');assert a.stop_nn_attack();s.update(phase='ended',time=t)
