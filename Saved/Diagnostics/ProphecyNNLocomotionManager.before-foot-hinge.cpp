...
void AProphecyNNLocomotionManager::ApplyOutputBatch(float StepSeconds)
{
	for (int32 AgentIndex = 0; AgentIndex < CrowdSize; ++AgentIndex)
	{
			...
			FVector2f& RawPin, FVector2f& EffectivePin, float* PrePinReference)
		{
			...
			if (Tempering)
			{
				if (bReconstructTemperedLegs) FMemory::Memcpy(PrePinReference, Transition, StateDim * sizeof(float));
		...
		if (bReconstructTemperedLegs)
		{
			...
			for (int32 I = 0; I < 2; ++I)
			{
				...
				ResolveTemperedLeg(*Tempering, StateSlice(Impl->PreviousPublishedStateBuffer, AgentIndex),
					TemperedReference, G, Transition, O);
			}
		}
