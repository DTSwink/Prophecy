...
folder=pathlib.Path(unreal.Paths.project_saved_dir())/'Diagnostics/DefenseContactModes';folder.mkdir(parents=True,exist_ok=True)
...
def launch(mode):
 ...
 assert d.set_simulation_mode(unreal.ProphecyAgentSimulationMode.PHYSICAL)
 d.set_editor_property('auto_publish_manual_follower_substep_targets',False)
 target=point(d)
 ...
 R.set_root_magic_velocity(d,unreal.Vector(),False)
 R.set_root_magic_ang_velocity(d,unreal.Vector(),False)
...
def tick(dt):
 try:
  ...
  for p in s.get('actors',[]):
   if p.get_simulation_mode()==unreal.ProphecyAgentSimulationMode.PHYSICAL:p.publish_manual_follower_substep_targets(dt)
   else:p.apply_nn_pose_kinematically(dt)
  if s['phase']=='settle':
  ...
  if status.active:
   ...
   s['seen_armed']=True
   assert not status.blocked and not status.harmful_contact and status.contact_time_seconds<0,'Sim used predicted contacts'
  ...
  if False:
   ...
   s['flipped']=True
  if status.completed_steps>=6 or (s['seen_armed'] and not status.active and status.completed_steps):
   assert status.completed_steps>=6 and status.active,('physical defense ended early',status.completed_steps,status.harmful_contact)
   D.stop_nn_defense(d);assert tuple(a.get_nn_attack_defense_state())==(False,False)
