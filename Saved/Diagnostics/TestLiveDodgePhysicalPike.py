"""Real scene NN attack + dodge; changes only disposable PIE actors."""
import unreal, pathlib, json, time, traceback, math
ed=unreal.get_editor_subsystem(unreal.UnrealEditorSubsystem)
level=unreal.get_editor_subsystem(unreal.LevelEditorSubsystem)
assert not ed.get_game_world()
folder=pathlib.Path(unreal.Paths.project_saved_dir()) /'Diagnostics/LiveDodgePhysicalPike'
...
def finish(result,error=''):
    ...
    report={k:v for k,v in s.items() if k in ('rows','agents','initial_distance_cm','max_hand_motion_cm','max_attacker_hand_motion_cm','final_steps','leg_lengths','max_handoff_hand_step_cm')}
    report.update(result=result,error=error,scope='Two actual testNN PIE actors, live attack and dodge NN, Jolt physical following; explicit target publication with unrelated Blueprint debug Tick disabled only in PIE; no prerecorded poses.')
    ...
    try:
        ...
        if s['phase']=='start':
            ...
            s.update(defender=d,attacker=a,actors=actors,agents=[a.get_name(),d.get_name()],
                     initial_distance_cm=(point(d,'pelvis')-point(a,'pelvis')).length(),phase='settle',time=t)
        for a in s.get('actors',[]):
            assert a.is_jolt_physical_animation_enabled()
            a.publish_manual_follower_substep_targets(dt)
        if s['phase']=='settle' and t-s['time']>.2:
            a,d=s['attacker'],s['defender']
            assert a.trigger_nn_attack('pike',point(d,'spine_03'),False),'Attack failed'
            ...
            s.update(phase='active',time=t,initial_hand=point(d,'hand_r'),initial_attack_hand=point(a,'hand_r'),
            ...
            physical=d.get_physical_body_state('hand_r');assert physical and physical[-1]
            assert all(math.isfinite(v) for v in vec(physical[0].translation)), 'Nonfinite physical hand'
            if 'physical_start' not in s:s['physical_start']=physical[0].translation
            s['physical_motion_cm']=(physical[0].translation-s['physical_start']).length()
            if status.completed_steps:
                lengths=[(point(d,x)-point(d,y)).length() for x,y in [('thigh_l','calf_l'),('calf_l','foot_l'),('thigh_r','calf_r'),('calf_r','foot_r')]]
                if 'leg_lengths' not in s:s['leg_lengths']=lengths
                assert max(abs(x-y) for x,y in zip(lengths,s['leg_lengths']))<.1,('Dodge changed leg lengths',lengths,s['leg_lengths'])
            ...
            if status.completed_steps!=s['last_steps']:
                s['rows'].append(dict(time=t,steps=status.completed_steps,attacker_frame=status.attacker_frame,
                    ...
                    contact=str(status.contact_collider),contact_seconds=status.contact_time_seconds,physical_motion_cm=s['physical_motion_cm']))
                s['last_steps']=status.completed_steps
            if (not status.active) or t-s['time']>.8:
                assert status.completed_steps>=3,('too few defense steps',status.completed_steps)
                assert s['max_hand_motion_cm']>.05 and s['max_attacker_hand_motion_cm']>.05,'No actual pose motion'
                assert s['physical_motion_cm']>.05, 'Physical hand did not move'
                api.stop_nn_defense(d);s.update(phase='stopped',time=t,final_steps=status.completed_steps,
                    final_root=d.get_root_low_point(),final_hand=point(d,'hand_r'),max_handoff_hand_step_cm=0.,last_hand=point(d,'hand_r'))
        elif s['phase']=='stopped':
            now=point(s['defender'],'hand_r')
            s['max_handoff_hand_step_cm']=max(s['max_handoff_hand_step_cm'],(now-s['last_hand']).length());s['last_hand']=now
            if t-s['time']<=.3:return
            assert s['max_handoff_hand_step_cm']<50,('Discontinuous handoff',s['max_handoff_hand_step_cm'])
