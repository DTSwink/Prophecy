...
if mode=='jolt_conditioned': unreal.SystemLibrary.execute_console_command(sub.get_editor_world(),'Prophecy.Jolt.Diagnostic.InertiaConditioning 1')
out = pathlib.Path(unreal.Paths.project_saved_dir())/'Diagnostics/FeedbackHandDrift'
out.mkdir(parents=True, exist_ok=True)
state = dict(rows=[], agents=[], metadata=[], handle=None, start=None, last=None, wall=time.perf_counter())
builtins._feedback_hand_drift = state
...
def finish(reason):
    ...
    if mode=='jolt_conditioned': unreal.SystemLibrary.execute_console_command(sub.get_editor_world(),'Prophecy.Jolt.Diagnostic.InertiaConditioning 0')
    ...
    try:
        ...
        if not state['agents']:
            ...
            for a in agents:
                ...
                state['metadata'].append(dict(actor=a.get_name(), mode=str(a.get_simulation_mode()),
                    ...
                    feedback={b:str(a.get_physical_feedback_tolerance(b)) for b in ('hand_l','hand_r','lowerarm_l','lowerarm_r','upperarm_l','upperarm_r')}, mesh=str(a.get_pose_reference_mesh().get_collision_profile_name())))
        ...
        for a in state['agents']:
            ...
            for b in ('pelvis','spine_05','upperarm_l','upperarm_r','lowerarm_l','lowerarm_r','hand_l','hand_r'):
                ...
                row['bones'][b]=dict(p=vec(tr.translation),q=quat(tr.rotation),v=vec(v),w=vec(w),target=vec(target.translation),tq=quat(target.rotation),future=vec(future[lookup[b]].translation),fq=quat(future[lookup[b]].rotation),sim=bool(sim))
            state['rows'].append(row)
        if t-state['start']>=(60 if mode=='jolt_final' else 25): finish('Complete')
