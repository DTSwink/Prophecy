...
void AProphecyNNLocomotionManager::ApplyOutputBatch(float StepSeconds)
{
	for (int32 AgentIndex = 0; AgentIndex < CrowdSize; ++AgentIndex)
	{
		...
		int32 SupportExperiment=0;
#if WITH_EDITOR
		if (bReconstructTemperedLegs) SupportExperiment=CVarTemperingSupportExperiment.GetValueOnGameThread();
#endif
		float ExperimentRunSource[StateDim],ExperimentWalkSource[StateDim];
			...
			FVector2f& RawPin, FVector2f& EffectivePin)
		{
			...
			if (SupportExperiment) FMemory::Memcpy(bWalkPolicy ? ExperimentWalkSource : ExperimentRunSource,Transition,StateDim*sizeof(float));
			if (Tempering)
			{
		...
		if (bReconstructTemperedLegs || (bRegional && !Tempering && ProphecyLegChainDebug::IsEnabled(AgentActors[AgentIndex])))
		{
			...
			for (int32 I = 0; I < 2; ++I)
			{
				...
				if (bReconstructTemperedLegs) ResolveTemperedLeg(*Tempering, StateSlice(Impl->PreviousPublishedStateBuffer, AgentIndex),
					...
					SupportExperiment ? (bWalkPolicy ? ExperimentWalkSource : ExperimentRunSource) : nullptr,SupportExperiment);
...
void AProphecyNNLocomotionManager::PublishAgentPose(int32 AgentIndex, double SourceTimeSeconds)
{
	...
	{
		DecodeLocomotionPose(Impl, Lower, Upper, WalkWeight, Out, Local, C,&LegWeights);
	};
	FImpl::FAgent& Agent = Impl->Agents[AgentIndex];
