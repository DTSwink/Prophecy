...
p=pathlib.Path(unreal.Paths.project_saved_dir())/'Diagnostics/WideWrist'
...
def finish(reason):
    ...
    (p/('bool_'+mode+'.json')).write_text(json.dumps({'reason':reason,'mode':mode,'switch_t':switch_t,'events':state['events'],'rows':state['rows']},separators=(',',':')))
    ...
    try:
        ...
        if not state['actors']:
            ...
            for a in state['actors']:
                unreal.SystemLibrary.execute_console_command(w,'prophecy.Diagnostic.BoolCodex '+a.get_name()+' 0')
                assert a.get_editor_property('Bool Codex')==False
        if mode!='false' and t>=switch_t and not state['switched']:
            for a in state['actors']:
                unreal.SystemLibrary.execute_console_command(w,'prophecy.Diagnostic.BoolCodex '+a.get_name()+' 1')
                assert a.get_editor_property('Bool Codex')==True,'Native Boolean write failed'
                state['events'].append({'t':t,'actor':a.get_name(),'bool':a.get_editor_property('Bool Codex')})
            state['switched']=True
            print('WRIST_BOOL_SWITCH',mode,t)
        for a in state['actors']:
            ...
            row={'t':t,'actor':a.get_name(),'player':str(a.get_controller()),'bool':a.get_editor_property('Bool Codex'),'bones':{},'limits':{b:str(lib.get_angular_limits(c)[1:]) for b,c in state['joints'][a.get_name()].items()}}
