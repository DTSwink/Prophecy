...
sys.argv=['BoolCapture.py', sys.argv[1] if len(sys.argv)>1 else 'true', sys.argv[2] if len(sys.argv)>2 else '10', sys.argv[3] if len(sys.argv)>3 else '35']
src=(pathlib.Path(unreal.Paths.project_saved_dir())/'Diagnostics/WideWrist/BoolCapture.py').read_text()
