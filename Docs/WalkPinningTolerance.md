...
The accepted Walk contract uses raw logits: two negative outputs pin both feet; otherwise the smaller raw output wins (equivalently, the larger decoded pinning probability). An exact tie selects the left foot. The original rule is preserved when Tolerance is zero.
