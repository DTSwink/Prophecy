...
`Set Attack Target Extra Reach(Agent, Extra Reach Cm=50)` changes X for that agent, shared across its attacks. Radius = measured GT radius + X. X must be finite and nonnegative; zero uses GT reach exactly. Setting 50 removes the override. Unknown attack names use zero horizontal reach rather than guessing a family. Invalid agent/target inputs pass through unchanged with zero Difference.
