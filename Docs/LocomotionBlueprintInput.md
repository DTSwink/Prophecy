...
| **Stop Locomotion Input** | Zeroes the stick, clears Run to request Walk, and preserves the facing target/scales. Uses normal mover braking and the configured Run → Walk policy blend; does not teleport, zero velocity or reset the blend. Explicit auto-run and attack-recovery overrides retain their normal priority. |
