...
- **Set Attack To Locomotion Hand Blend**: Left Hand Source/Hold/Blend, then Right Hand Source/Hold/Blend. Source is Normal, Walk or Run. The node defaults to Run/0/1; before it is configured, no hand recovery is active. Zero blend with positive hold keeps the selected source until the hold ends, then switches to normal. Normal or both durations zero disables that hand, matching leg recovery. Positive configuration is selected at the next special end; On Attack Ended/Special Ended can configure the current handoff before its first locomotion prediction.
