...
const FSettings* Find(const AProphecyAgent* Agent)
{
    ...
    auto SamplePart=[&](auto& Timelines,bool Feet,ProphecyBlendClock::EKind Kind)
    {
        ...
        if (Feet) { Value->FeetTranslation=Sample.FeetTranslation;Value->FeetRotation=Sample.FeetRotation; }
        else { Value->PelvisTranslation=Sample.PelvisTranslation;Value->PelvisRotation=Sample.PelvisRotation; }
        const bool Done=Feet ? Value->FeetTranslation==1 && Value->FeetRotation==1
            : Value->PelvisTranslation==1 && Value->PelvisRotation==1;
...
static bool BlendPart(AProphecyAgent* Agent,bool Feet,float Duration,float Hold)
{
    ...
    {
        if (Feet) Shared->Initial.FeetTranslation=Shared->Initial.FeetRotation=1;
        else Shared->Initial.PelvisTranslation=Shared->Initial.PelvisRotation=1;
    ...
    const bool AlreadyNormal=Feet ? Initial.FeetTranslation==1 && Initial.FeetRotation==1
        : Initial.PelvisTranslation==1 && Initial.PelvisRotation==1;
    if (AlreadyNormal || (Duration==0 && Hold==0))
    {
        ...
        if (Feet) Value.FeetTranslation=Value.FeetRotation=1;
        else Value.PelvisTranslation=Value.PelvisRotation=1;
    ...
    float FeetTranslation, float FeetRotation, float PelvisTranslation, float PelvisRotation)
{
    ...
    for (float V : {FeetTranslation, FeetRotation, PelvisTranslation, PelvisRotation})
        if (!FMath::IsFinite(V) || V < 0.f || V > 1.f) return false;
    const FSettings Value{FeetTranslation, FeetRotation, PelvisTranslation, PelvisRotation};
...
{
    using namespace ProphecyLowerTempering;
    FReturnTimeline T{{0,.2f,.4f,.6f},2,1};
    ...
    TestTrue(TEXT("Set initial"),L::SetLocomotionLowerBodyTempering(Agent,true,0,0,0,0));
    ...
    L::SetLocomotionLowerBodyTempering(Agent,true,0,0,0,0);
    L::BlendLocomotionLowerBodyTemperingToNormal(Agent,2,1,2,1);
    L::SetLocomotionLowerBodyTempering(Agent,true,.4f,.4f,.4f,.4f);
...
bool FProphecyTemperingSeparateReturnsTest::RunTest(const FString&)
{
    ...
    L::SetLocomotionLowerBodyTempering(Agent,true,0,0,.4f,.6f);
