...
struct FCell
{
    ...
    void Sample(double Clock)
    {
        ...
        const double T=FMath::Clamp((Clock-Begin)/Duration,0.,1.);
        Value={FMath::Lerp(Start.Scales,Target.Scales,float(T*T*(3.-2.*T))),true};
...
{
    const double Now=Agent.GetWorld()->GetTimeSeconds();
    State.Clock+=FMath::Max(0.,Now-State.WorldTime)*FMath::Max(0.f,Agent.CustomTimeDilation);
    State.WorldTime=Now;
}
...
static void UpdateState(AProphecyAgent& Agent,FAgentState& State,float Walk,bool Drawn,bool Attack)
{
    AdvanceClock(Agent,State);
    const bool Changed=!State.ValidContext || State.Walk!=Walk || State.Drawn!=Drawn || State.Attack!=Attack;
    if (!Changed && !State.Running) return;
    State.Running=false;
...
void Update(AProphecyAgent* Agent)
{
    ...
    UpdateState(*Agent,*State,FMath::Clamp(Walk,0.f,1.f),Drawn,Attack);
}
    ...
    EProphecyLocomotionSelection L,EProphecyEquipmentSelection E)
{
    ...
    {
        State->Entries.RemoveAllSwap([&](const FEntry& X) { return X.Bone==Bone && X.Kind==Kind; });
...
bool FProphecyPhysicalContextTest::RunTest(const FString&)
{
    ...
    FWorldDelegates::OnWorldPreActorTick.Broadcast(World,LEVELTICK_All,.25f);
    Agent->NotifySwordAttackState(true);
