...
static auto& Storage(int32 Checkpoint) { return Checkpoint==2?Refresh2Comparisons:Comparisons; }
...
{
    for(int32 I=1;I<=2;++I) if(const auto* C=Find(Owner,I))
...
{
    for(int32 I=1;I<=2;++I) if(auto* C=Find(Owner,I))
    { auto& Set=Active?C->Active:C->Selected;if(I==Choice)Set.Add(Actor);else Set.Remove(Actor); }
}
static void Clear(const AProphecyNNLocomotionManager* Owner) { Comparisons.Remove(Owner);Refresh2Comparisons.Remove(Owner); }
}
